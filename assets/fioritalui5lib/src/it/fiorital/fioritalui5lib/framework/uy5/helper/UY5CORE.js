 ...
 sap.ui.define(["sap/ui/base/ManagedObject", "sap/base/Log", "sap/m/MessageBox", "sap/ui/core/format/DateFormat",
 	...
 	function (ManagedObect, Log, MessageBox, DateFormat, DYNMODEL, JSONModelExt, cryptojs) {
 		...
 		return ManagedObect.extend('it.fiorital.fioritalui5lib.framework.uy5.helper.UY5CORE', {
 			...
 			constructor: function (componentRef, disableRegexpEscapigOnJSONResults, sendXsession, observeChanges) {
 				...
 				this.sStreamParams = [];
 				this.SyncModels = [];
 				this.sModel = componentRef.getModel();
 			...
 			addParamStructure: function (as, varval) {
 				var value = btoa(JSON.stringify(varval));
 				this.addSinglePar(as, value);
 				return this;
 			},
 			...
 			getXMLModels: function (success, fail, persisteModels) {
 				...
 				var baseurl = this.component.getMetadata().getComponentName();
 				baseurl = baseurl.split('.').join('/');

 				var XMLurl = baseurl + '/model/DYNM_SAP_models.xml';
 				XMLurl = sap.ui.require.toUrl(XMLurl);
 			...
 			addSyncModel: function (modelName, way, explicitModel, structure, syncGroups, binders, type) {
 				...
 				if (explicitModel === null) {
 					newSyncModel['JSMODEL'] = new JSONModelExt(undefined, newSyncModel,undefined,undefined,this.observeChanges);
 					//newSyncModel['JSMODEL'].setPagination(1);   //<--- HT TEST TEST
 					newSyncModel['JSMODEL'].setSizeLimit(100000);

 					this.component.setModel(newSyncModel['JSMODEL'], newSyncModel['NAME']);
 				} else {
 				...
 				eval("window." + modelName + " = newSyncModel['WRAPPER'];");
 			...
 			_INTERNAL_linkBinderTable: function (binder, SyncModel) {
 				...
 				//---> get UI control
 				var finalId = this.component.getId() + '---' + binder["CTRLID"];
 				var ctrl = sap.ui.getCore().byId(finalId);
 				if (ctrl !== null) {
 					//---> set model & bind
 					ctrl.setModel(SyncModel['JSMODEL']);

 					if (binder["FILTER"] !== undefined && binder["FILTER"] !== '') {

 						var fltData = binder["FILTER"].split(";");
 						var flt = new sap.ui.model.Filter(fltData[0], eval(fltData[1]), fltData[2]);
 						ctrl.bindAggregation(binder["AGGREGATION"], {
 							path: "/",
 							filters: flt,
 							template: ctrl.getAggregation(binder["AGGREGATION"])[0].clone()
 						});

 					} else {

 						if (binder["NOAGGREGATIONMODEL"] === undefined || binder["NOAGGREGATIONMODEL"] === '') {

 							var mdl = ctrl.getAggregation(binder["AGGREGATION"]);
 							if (mdl === null) {

 								try {
 									mdl = eval('ctrl.get' + binder["AGGREGATION"] + '()');
 								} catch (e) {
 									//--> go on
 								}

 								if (mdl === null) {
 									var capitalized = binder["AGGREGATION"].replace(/(^|\s)([a-z])/g, function (m, p1, p2) {
 										return p1 + p2.toUpperCase();
 									});

 									try {
 										mdl = eval('ctrl.get' + capitalized + '()');
 									} catch (e) {
 										//--> merda
 										return false;
 									}
 							...
 							if (mdl !== null) {
 								if (mdl.length === 0) {
 									MessageBox.alert("DYN development BUG: not defined item template for aggregation in control: '" + binder["CTRLID"] + "'.");
 									return false;
 								} else {
 									ctrl.bindAggregation(binder["AGGREGATION"], "/", mdl[0].clone());
 								}
 							}
 						}

 						//---> late set path
 						if (binder["LATEPATH"] !== null && binder["LATEPATH"] !== undefined && binder["LATEPATH"] !== '') {
 							var ctx = new sap.ui.model.Context(SyncModel['JSMODEL'], binder["LATEPATH"]);
 							ctx.setForceRefresh(true);

 							var bnd = ctrl.getBinding(binder["AGGREGATION"]);
 							bnd.setContext(ctx);
 						}

 					} //<--- bind aggregation directly only if row model is provided (not used for example for sap.ui.table where model set is enought)

 					try {
 						ctrl.getBinding(binder["AGGREGATION"]).initialize();
 					} catch (e) {
 						//---> nothing to do	
 					}

 					SyncModel['JSMODEL'].refresh();
 					return true;
 				} else {
 					return false;
 				}
 			...
 			_INTERNAL_linkBinder: function (binder, SyncModel) {
 				...
 				if (SyncModel['TYPE'] === undefined || SyncModel['TYPE'] === null || SyncModel['TYPE'] === 'table') {
 					var res = this._INTERNAL_linkBinderTable(binder, SyncModel);
 					if (res === true) {
 						binder["binded"] = true;
 					}
 				}

 				if (SyncModel['TYPE'] === 'structure') {

 					//---> reference the pointer
 					var baseFakeArray = [];
 					baseFakeArray.push(SyncModel['STRUCTUREDATA']);
 					SyncModel['JSMODEL'].setData(baseFakeArray);

 					var res = this._INTERNAL_linkBinderStructure(binder, SyncModel);
 					if (res === true) {
 						binder["binded"] = true;
 					}
 				}
 			...
 			_INTERNAL_linkBinders: function (binders, SyncModel, lateBinding) {
 				...
 				if (SyncModel['TYPE'] === undefined || SyncModel['TYPE'] === null || SyncModel['TYPE'] === 'table') {
 					for (var idx = 0; idx < binders.length; idx++) {
 						if ((binders[idx]['binded'] === false || binders[idx]['binded'] === undefined) &&
 							((lateBinding === false && (binders[idx]['LATE'] === '' || binders[idx]['LATE'] === undefined)) ||
 								(lateBinding === true && (binders[idx]['LATE'] === 'X')))) {
 							var res = this._INTERNAL_linkBinderTable(binders[idx], SyncModel);
 							if (res === true) {
 								binders[idx]['binded'] = true;
 							}
 						}
 					}
 				}
 				...
 				if (SyncModel['TYPE'] === 'structure') {
 					...
 					for (var idx = 0; idx < binders.length; idx++) {
 						if ((binders[idx]['binded'] === false || binders[idx]['binded'] === undefined) &&
 							((lateBinding === false && (binders[idx]['LATE'] === '' || binders[idx]['LATE'] === undefined)) ||
 								(lateBinding === true && (binders[idx]['LATE'] === 'X')))) {
 							var res = this._INTERNAL_linkBinderStructure(binders[idx], SyncModel);
 							if (res === true) {
 								binders[idx]['binded'] = true;
 							}
 						}
 						...
 						for (var key in this.thisRef.ResultRaw[idx]) {
 							...
 							if (isModel !== null) {
 								...
 								if (isModel.TYPE === 'table') {
 									...
 									if (isModel.WRAPPER.AutoUpdate === true) {
 										isModel.JSMODEL.refresh();
 									}
 								}
