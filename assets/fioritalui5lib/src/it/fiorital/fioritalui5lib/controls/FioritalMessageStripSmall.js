...
sap.ui.define([
	...
	"use strict";
	return Control.extend("it.fiorital.fioritalui5lib.controls.FioritalMessageStripSmall", {
		...
		constructor: function (message, paramsObject, parentObject) {
			...
			this._Element.classList.add("js-snackbar__wrappersmall");
			...
			this._Message.classList.add("js-snackbar__messagesmall");
		...
		init: function () {
			...
			if (sap.ushell !== undefined && sap.ushell.Container !== undefined && sap.ushell.Container.getService("AppLifeCycle") !== undefined) {
				...
				try {
					...
					baseurl = 'sap/fiori/' + appName + '/resources/' + baseurl.split('.').join('/') + '/FioritalMessageStripSmall.css';
					...
					try { // Prevent dump on SCP
						var baseurl = this.getMetadata().getLibraryName();
						baseurl = '../zfioritalui5lib/controls/FioritalMessageStripSmall.css';
						includeStylesheet(baseurl);
					} catch (ex) {}
					...
					try { // Prevent dump on ABAP stack
						var baseurl = this.getMetadata().getLibraryName();
						baseurl = 'resources/' + baseurl.split('.').join('/') + '/FioritalMessageStripSmall.css';
						includeStylesheet(baseurl);
					} catch (ex) {}
				}
			} else {
