...
sap.ui.define([
	...
	return ManagedObject.extend("it.fiorital.fioritalui5lib.controls.APCmanager", {
		...
		constructor: function (urlHandShake, urlWebSocket, componentRef, modelName, randomAPC) {
			...
			this.randomAPC = randomAPC;

			this.appId = componentRef.getManifest()['sap.app'].id;
			this.__internal_set_header(componentRef,'fioritalapp',this.appId);

			this.urlHandShake = urlHandShake;
			...
			this.jsonYsocket.attachRequestCompleted(function (evt) {
				...
				if (this.randomAPC) {  //<-- random APC cahnnel selection 
			...
			if (this.modelName === '') {
				this.jsonYsocket.loadData(componentRef.getModel().sServiceUrl + '/Ysocket');
			} else {
				this.jsonYsocket.loadData(componentRef.getModel(this.modelName).sServiceUrl + '/Ysocket');
			}

			componentRef.YsocketManager = this;
		},
